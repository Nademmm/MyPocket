<x-app-layout title="Edit Savings Target">
    <div class="max-w-xl mx-auto">
        ...
        <div class="bg-gradient-to-br from-white to-[#faf8ed] border border-[#c5d89d]/30 rounded-3xl p-6 md:p-8 shadow-xl relative overflow-hidden">
            ...
            <form method="POST" action="{{ route('targets.update', $target->id) }}" enctype="multipart/form-data" class="space-y-8 relative">
                @csrf
                @method('PUT')
                        ...
                        <div class="flex items-center gap-4">
                            ...
                            <div class="w-40 aspect-video rounded-2xl border-2 border-dashed border-[#c5d89d]/40 flex items-center justify-center overflow-hidden bg-white shadow-sm">
                                <template x-if="!photoPreview">
                                    @if($target->image)
                                        <img src="{{ asset('storage/' . $target->image) }}" class="w-full h-full object-cover">
                                    @else
                ...
                <div class="space-y-3">
                    ...
                    <div class="flex items-center gap-4 p-4 bg-white border-2 border-[#c5d89d]/20 rounded-2xl shadow-sm">
                        ...
                        <label class="flex items-center gap-3 cursor-pointer group">
                            <input type="checkbox" name="is_published" value="1" class="w-5 h-5 ml-[50px] mr-[50px] rounded border-[#c5d89d] text-[#9cab84] focus:ring-[#9cab84] transition-all cursor-pointer" {{ old('is_published', $target->is_published) ? 'checked' : '' }}>
                        </label>
                    </div>
                </div>
