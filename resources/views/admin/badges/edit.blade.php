<x-app-layout title="Edit Badge">
    <div class="max-w-2xl mx-auto">
        <div class="mb-8 flex items-center justify-between">
            <div>
                <h1 class="text-3xl font-bold text-gray-800">Edit Achievement</h1>
                <p class="text-gray-600">Update badge details and requirements</p>
            </div>
        ...
        <form action="{{ route('admin.badges.update', $badge->id) }}" method="POST" enctype="multipart/form-data" class="bg-white rounded-2xl shadow-sm border border-gray-100 p-8">
            @csrf
            @method('PUT')
            ...
            <div class="space-y-6">
                ...
                <div>
                    ...
                    <textarea name="description" id="description" rows="3" required
                              class="w-full px-4 py-3 bg-gray-50 border border-gray-200 focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 rounded-xl transition-all outline-none font-medium text-gray-800">{{ old('description', $badge->description) }}</textarea>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <select name="requirement_type" id="requirement_type" required
                                class="w-full px-4 py-3 bg-gray-50 border border-gray-200 focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 rounded-xl transition-all outline-none font-bold text-gray-800">
                            <option value="target_count" {{ old('requirement_type', $badge->requirement_type) == 'target_count' ? 'selected' : '' }}>Completed Targets</option>
                            <option value="transaction_count" {{ old('requirement_type', $badge->requirement_type) == 'transaction_count' ? 'selected' : '' }}>Total Transactions</option>
                            <option value="total_savings" {{ old('requirement_type', $badge->requirement_type) == 'total_savings' ? 'selected' : '' }}>Total Savings (Rp)</option>
                        </select>
                    ...
                    <div>
                        ...
                        <input type="number" name="requirement_value" id="requirement_value" value="{{ old('requirement_value', $badge->requirement_value) }}" required min="0" step="0.01"
                               class="w-full px-4 py-3 bg-gray-50 border border-gray-200 focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 rounded-xl transition-all outline-none font-bold text-gray-800">
                ...
                <!-- Image -->
                <div>
                    <label for="image" class="block text-xs font-bold text-gray-400 uppercase tracking-widest mb-2 ml-1">Update Icon (Optional)</label>
