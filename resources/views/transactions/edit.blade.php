<x-app-layout title="Edit Transaction">
    ...
    <div class="mb-8 flex items-center justify-between">
        <div>
            <h1 class="text-3xl font-bold text-blue-900 mb-1">Edit Transaction</h1>
            <p class="text-gray-500 text-sm">Update your transaction details</p>
        </div>
        ...
        <form method="POST" action="{{ route('transactions.update', $transaction->id) }}" class="space-y-6">
                ...
                <div class="grid grid-cols-2 gap-3">
                    ...
                    <label id="label-income"
                           class="flex items-center gap-3 p-3.5 bg-gray-50 border border-gray-200 rounded-xl cursor-pointer hover:border-emerald-400 transition
                                  {{ $currentType == 'income' ? 'border-emerald-500 bg-emerald-100' : '' }}">
                    ...
                    <label id="label-expense"
                           class="flex items-center gap-3 p-3.5 bg-gray-50 border border-gray-200 rounded-xl cursor-pointer hover:border-red-400 transition
                                  {{ $currentType == 'expense' ? 'border-red-500 bg-red-100' : '' }}">
            ...
            <div>
                ...
                <textarea id="description" name="description" rows="3" placeholder="Enter description..."
                          class="w-full px-4 py-3 bg-gray-50 border border-gray-200 text-gray-800 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition placeholder-gray-500 resize-none">{{ old('description', $transaction->description) }}</textarea>
                @error('description')
                    <p class="mt-1.5 text-sm text-red-500">{{ $message }}</p>
                @enderror
            </div>
