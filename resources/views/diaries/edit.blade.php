<x-app-layout title="Edit Diary">
    <div class="max-w-3xl mx-auto">
        <!-- Header -->
        <div class="mb-10 flex items-center justify-between">
            <div>
                <h1 class="text-3xl font-black text-[#2d2d2d] mb-2 tracking-tight">Edit Your Diary.</h1>
                <p class="text-[#89986d] font-medium">Update your financial reflection.</p>
            </div>
        ...
        <div class="bg-white border border-[#c5d89d]/30 rounded-[2.5rem] p-8 md:p-12 shadow-2xl relative overflow-hidden">
            ...
            <form method="POST" action="{{ route('diaries.update', $diary->id) }}" class="space-y-8 relative">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
                    ...
                    <div class="space-y-3">
                        ...
                        <input type="date" id="diary_date" name="diary_date" value="{{ old('diary_date', $diary->diary_date->toDateString()) }}" required 
                               class="w-full px-6 py-4 bg-[#faf8ed]/50 border-2 border-[#c5d89d]/20 focus:border-[#c5d89d] text-[#2d2d2d] font-bold rounded-2xl transition-all outline-none shadow-sm"/>
                        @error('diary_date')
                            <p class="text-[10px] text-red-500 mt-1 ml-1 font-semibold uppercase tracking-wider">{{ $message }}</p>
                        @enderror
                ...
                <div class="space-y-3">
                    ...
                    <textarea id="content" name="content" rows="10" required minlength="10" 
                              class="w-full px-8 py-6 bg-[#faf8ed]/50 border-2 border-[#c5d89d]/20 focus:border-[#c5d89d] text-[#2d2d2d] font-medium rounded-[2rem] transition-all outline-none shadow-sm resize-none leading-relaxed">{{ old('content', $diary->content) }}</textarea>
